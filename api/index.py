...
# 1. Standard CORS Setup for Vercel
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
@app.get("/api")
def health_check():
    return {"status": "alive", "service": "IntentX Backend"}

@app.post("/api/predict")
async def predict(request: AnalysisRequest):
    # Running the analyzer twice as per your logic to get cumulative score
    l_score, l_reasons = analyze_links(request.message)
    i_score, i_reasons = analyze_links(request.message) 
    
    total_score = l_score + i_score
    all_reasons = list(set(l_reasons + i_reasons)) # Use set to remove duplicates
...
# Vercel handles the app execution. No uvicorn.run() needed here.